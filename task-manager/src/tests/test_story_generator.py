#!/usr/bin/env python3
"""
Test script for StoryGenerator
Tests story generation with Gojko Adzic patterns integration
"""

import sys
import os
sys.path.append('/home/pauljcoles/code/cats/task-manager/base-rules/code-rules')

# Change to the correct directory for imports
os.chdir('/home/pauljcoles/code/cats/task-manager')

from base_rules.code_rules.story_generator import StoryGenerator
from base_rules.code_rules.specification_parser import SpecificationParser

def test_story_generator():
    """Test the story generation functionality"""
    
    print("🧪 Testing StoryGenerator with Gojko Patterns")
    print("=" * 60)
    
    # Initialize components
    parser = SpecificationParser()
    generator = StoryGenerator()
    
    # Load and parse the test specification
    spec_file = "/home/pauljcoles/code/cats/task-manager/specifications/SPECMERCEDES-001.md"
    
    try:
        with open(spec_file, 'r') as f:
            content = f.read()
        
        print(f"📋 Loaded specification content")
        
        # Parse specification  
        spec_data = parser.parse_specification(content, "SPECMERCEDES-001")
        print(f"✅ Parsed specification: {spec_data.title}")
        print(f"   - Personas: {len(spec_data.personas)}")
        print(f"   - Business Goals: {len(spec_data.business_goals)}")
        
        # Generate stories
        print(f"\n🎯 Generating user stories...")
        stories = generator.generate_stories_from_spec(spec_data, "mercedes")
        
        if stories:
            print(f"\n✅ Generated {len(stories)} user stories!")
            
            # Show story details
            for i, story in enumerate(stories[:3], 1):  # Show first 3
                print(f"\n📖 Story {i}: {story.title}")
                print(f"   ID: {story.story_id}")
                print(f"   Description: {story.description}")
                print(f"   INVEST Score: {story.invest_score:.2f}")
                print(f"   Priority: {story.priority}")
                print(f"   Story Points: {story.story_points}")
                print(f"   Labels: {', '.join(story.labels)}")
                
                # Show acceptance criteria
                if story.acceptance_criteria:
                    print(f"   Acceptance Criteria:")
                    for j, ac in enumerate(story.acceptance_criteria, 1):
                        print(f"     {j}. Given: {ac.given}")
                        print(f"        When: {ac.when}")  
                        print(f"        Then: {ac.then}")
                        print(f"        Priority: {ac.priority}")
                
                # Show quality notes
                if story.quality_notes:
                    print(f"   Quality Notes:")
                    for note in story.quality_notes:
                        print(f"     • {note}")
            
            # Show summary
            avg_invest_score = sum(s.invest_score for s in stories) / len(stories)
            print(f"\n📊 Story Quality Summary:")
            print(f"   Average INVEST Score: {avg_invest_score:.2f}")
            print(f"   Stories with full INVEST: {sum(1 for s in stories if s.invest_score >= 0.8)}")
            print(f"   Stories needing improvement: {sum(1 for s in stories if s.invest_score < 0.6)}")
            
            return True
        else:
            print("❌ No stories generated!")
            return False
        
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
        traceback.print_exc()
        return False

if __name__ == "__main__":
    success = test_story_generator()
    sys.exit(0 if success else 1)