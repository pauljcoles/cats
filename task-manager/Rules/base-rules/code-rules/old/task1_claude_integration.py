...
Enhanced with focused contextual patterns for LLM analysis
"""
...
class ClaudeIntegratedEvaluator:
    ...
    def __init__(self):
        # Load focused LLM contextual patterns
    ...
    def load_contextual_patterns(self) -> Dict:
        """Load focused LLM contextual patterns from JSON"""
        try:
            pattern_file = "/home/pauljcoles/code/cats/task-manager/knowledge/llm-contextual-patterns.json"
            with open(pattern_file, 'r') as f:
                return json.load(f)
        except FileNotFoundError:
            print("⚠️ LLM contextual patterns file not found, using fallback patterns")
            return {"contextual_analysis_frameworks": {}}
    ...
    def llm_check_multiple_behaviors(self, ac_id: str, ac_text: str) -> Optional[LanguageIssue]:
        ...
        # Get focused contextual patterns for behavior analysis
        behavior_patterns = self.contextual_patterns.get("contextual_analysis_frameworks", {}).get("gojko_behavior_change", {})
        
        analysis_request = f"""
Analyze this acceptance criterion using focused contextual patterns for behavior change:

ACCEPTANCE CRITERION: "{ac_text}"

CONTEXTUAL ANALYSIS FOCUS: {behavior_patterns.get('analysis_focus', 'Does this describe one specific behavior change or multiple?')}

REASONING PROMPTS:
"""
        
        # Add reasoning prompts from contextual patterns
        reasoning_prompts = behavior_patterns.get('reasoning_prompts', [
            "Would a user see this as one coherent action or multiple separate actions?",
            "If this story fails, would it fail for one reason or multiple reasons?", 
            "Can this be independently valuable without other stories?"
        ])
        
        for i, prompt in enumerate(reasoning_prompts, 1):
            analysis_request += f"{i}. {prompt}\n"
        
        analysis_request += f"""
CONFIDENCE INDICATORS:
- High confidence (0.82-0.85): {behavior_patterns.get('confidence_indicators', {}).get('high_confidence', 'Clear compound behaviors with distinct outcomes')}
- Medium confidence (0.75-0.82): {behavior_patterns.get('confidence_indicators', {}).get('medium_confidence', 'Sequential actions that might be single workflow')}

Please respond with your analysis in this JSON format:
```json
{{
  "framework_applied": "gojko_behavior_change",
